...
// thanks to https://github.com/Code-Stage/Package2Folder/blob/master/Editor/Package2Folder.cs
public class BatchImportAssetPackages : ScriptableWizard
{
    [MenuItem("Assets/Import Package/Import Packages Here...")]
    public static void CreateWizard()
    {
        string selectedFolderPath = null;
        if (Selection.assetGUIDs == null || Selection.assetGUIDs.Length == 0)
        {
            Debug.LogError("Please select a target folder in the Project window where you wish to import packages into.");
            return;
        }

        var assetGuid = Selection.assetGUIDs[0];
        var path = AssetDatabase.GUIDToAssetPath(assetGuid);
        selectedFolderPath = !Directory.Exists(path) ? null : path;
        ...
        string packagePath = EditorUtility.OpenFolderPanel("Import Packages From WhichFolder?", "", "");
        ...
        foreach (string curPath in allFilePaths)
        {
            try
            {
                ...
                if (Path.GetExtension(fileToImport).ToLower() == ".unitypackage")
                {
                    ...
                    ImportPackageToFolder(fileToImport, selectedFolderPath, false);
                    packageCount++;
                }
            }
        ...
        Debug.Log("Import complete. " + packageCount + " packages imported, " + failCount + " failed.");
    }
